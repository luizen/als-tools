...
public class MidiEffectRackDeviceExtractor : BaseStockDeviceExtractor, IStockRackDeviceExtractor
{
    ...
    public MidiEffectRackDeviceExtractor(ILogger<MidiEffectRackDeviceExtractor> logger, Lazy<IDictionary<DeviceType, IDeviceTypeExtractor>> deviceTypeExtractors) : base(logger)
    {
    ...
    {
        var device = (MidiEffectRackDevice)base.ExtractFromXml(deviceNode);
    ...
    {
        logger.LogDebug("----");
        logger.LogDebug("Exctracting devices of MidiEffectRackDevice chains from XML...");

        var devices = new List<IDevice>();
        var devicesIterator = nav.Select(@"./Branches/MidiEffectBranch/DeviceChain/MidiToMidiDeviceChain/Devices/*");
