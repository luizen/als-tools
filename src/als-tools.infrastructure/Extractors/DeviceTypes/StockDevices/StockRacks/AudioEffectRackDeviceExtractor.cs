...
public class AudioEffectRackDeviceExtractor : BaseStockDeviceExtractor, IStockRackDeviceExtractor
{
    ...
    public AudioEffectRackDeviceExtractor(ILogger<AudioEffectRackDeviceExtractor> logger, Lazy<IDictionary<DeviceType, IDeviceTypeExtractor>> deviceTypeExtractors) : base(logger)
    {
    ...
    {
        var device = (AudioEffectRackDevice)base.ExtractFromXml(deviceNode);
    ...
    {
        logger.LogDebug("----");
        logger.LogDebug("Exctracting devices of AudioEffectRack chains from XML...");

        var devices = new List<IDevice>();
        var devicesIterator = nav.Select(@"./Branches/AudioEffectBranch/DeviceChain/AudioToAudioDeviceChain/Devices/*");
